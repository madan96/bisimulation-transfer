...
src_qval = np.load('optimal_qvalues_8_new_prob_states.npy')
# tgt_qval = np.zeros((tgt_env.state_space, 4))

src_agent.qvalues = src_qval
# tgt_agent.qvalues = tgt_qval
...
def compute_d():
    ...
    # u_bound = (-1, 1)
    # v_bound = (-1, 1)
    d = np.zeros((src_state_space - len(src_blocked_positions), action_space, tgt_state_space - len(tgt_blocked_positions), action_space))
...
def compute_dl(d):
    ...
    """
    S1 = src_env.state_space - len(src_blocked_positions)
    S2 = tgt_env.state_space - len(tgt_blocked_positions)
...
laxBisimTransfer(src_state_space - len(src_blocked_positions), tgt_state_space - len(tgt_blocked_positions))
